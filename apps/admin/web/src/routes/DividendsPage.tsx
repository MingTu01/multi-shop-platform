...
export function DividendsPage() {
  ...
  const { data, loading } = useApi<any[]>(storeId ? `/dividends?storeId=${storeId}` : null, [storeId]);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <h1 className="text-xl font-bold text-slate-800">分红管理 - {storeId}</h1>
      </div>
      <Card title="分红记录">
        <Table<any>
          loading={loading}
          rowKey={(r) => r.id}
          data={data || []}
          empty={<Empty title="暂无分红记录" />}
          columns={[
            { key: 'shareholder_name', title: '股东' },
